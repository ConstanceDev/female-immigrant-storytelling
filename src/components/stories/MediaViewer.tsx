...
export default function MediaViewer({ 
    ...
    // Early return if no media files
    ...
    useEffect(() => {
        const loadFileInfo = async () => {
            ...
            // Reset loaded files when mediaFiles change
            setLoadedFiles([])

            setLoading(true)
            try {
                // In a real app, you might batch this into a single API call
                const filePromises = mediaFiles.map(async (fileId) => {
                    ...
                    if (response.ok) {
                        ...
                        return {
                            ...
                            type: contentType,
                            uploadedAt: '',
                            url: `/api/files/${fileId}?v=fixed_${Date.now()}`
                        }
                    }
                    return null
                })
        ...
        // Only load if we have mediaFiles and not already loading
        loadFileInfo()
    }, [mediaFiles]) // Removed loading from dependencies to prevent loops
    ...
    const renderMedia = (file: FileInfo) => {
        if (file.type.startsWith('image/')) {
            return (
                <div className="cursor-pointer" onClick={() => setSelectedMedia(file)}>
                    <img
                        src={file.url}
                        alt={file.name}
